...
func _test_meshflow_live_integration() -> void:
	...
	if body != null:
		...
		if collision_shape != null:
			var is_valid_shape: bool = (collision_shape.shape is ConvexPolygonShape3D) or (collision_shape.shape is BoxShape3D)
			_assert("Live ConvexPolygonShape3D or BoxShape3D fallback used", is_valid_shape, "Shape is " + collision_shape.shape.get_class())
