...
const TEMP_DIR := "user://fovea_tools/temp"
...
func start_install(tool_name: String) -> void:
	...
	var ext := ".zip" if url.ends_with(".zip") else ".tar.gz"
	if url.ends_with(".tar.xz"):
		ext = ".tar.xz"
	var temp_file := TEMP_DIR.path_join(tool_name + ext)
	...
	var err := _active_http_request.request(url)
	if err != OK:
		_cleanup_active_request()
...
func _process(_delta: float) -> void:
	if _active_http_request and _active_tool != "":
		...
		if total > 0:
			...
			var speed_str := "%s / %s" % [_format_bytes(downloaded), _format_bytes(total)]
...
func _on_download_completed(result: int, response_code: int, _headers: PackedStringArray, temp_file_path: String) -> void:
	...
	thread.start(Callable(self, "_async_extract").bind(temp_file_path, tool_name))

func _async_extract(temp_file_path: String, tool_name: String) -> void:
	var temp_extract_dir := TEMP_DIR.path_join(tool_name + "_extracted")
	DirAccess.make_dir_recursive_absolute(temp_extract_dir)

	var global_archive := ProjectSettings.globalize_path(temp_file_path)
	var global_dest := ProjectSettings.globalize_path(temp_extract_dir)

	var output: Array = []
	print("FoveaDependencyInstaller: Extracting '%s' using tar..." % tool_name)
	var code := OS.execute("tar", ["-xf", global_archive, "-C", global_dest], output, true, false)
	...
	var final_dest_dir := TOOLS_DIR.path_join(tool_name)
	...
	if dir:
		...
		var source_dir := temp_extract_dir
...
func _async_python_post_install(python_dir: String) -> void:
	...
	var python_exe := python_dir.path_join("python.exe" if OS.get_name() == "Windows" else "bin/python")
	...
	var code := OS.execute(python_exe, ["-m", "pip", "install", "--upgrade", "pip"], [], true)
	if code != 0:
	...
	# Ensure WorldMirrorHunYuan requirements can also be satisfied locally in bridges
	install_completed.emit.call_deferred("python")

func _cleanup_active_request() -> void:
