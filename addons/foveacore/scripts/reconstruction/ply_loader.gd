...
static func load_gaussians_from_ply(path: String) -> Array[GaussianSplat]:
	...
	while line != "end_header":
		# Guard against a truncated header (missing end_header)
		if file.eof_reached():
	...
	for i: int in range(properties.size()):
		prop_map[properties[i]["name"]] = i

	if is_binary:
		for i: int in range(element_count):
			if file.eof_reached():
				break
			...
			for p: int in range(properties.size()):
		...
		for i: int in range(element_count):
			var vertex_line: String = file.get_line().strip_edges()
			if vertex_line.is_empty():
				if file.eof_reached():
					break
				continue
				
			var parts: PackedStringArray = vertex_line.split(" ", false)
...
static func _map_properties_to_splat(splat: GaussianSplat, data: Array[float], prop_map: Dictionary, properties: Array[Dictionary]) -> void:
	...
	if prop_map.has("f_dc_0") and prop_map.has("f_dc_1") and prop_map.has("f_dc_2"):
		...
		var red_val: float = data[prop_map["red"]]
		var green_val: float = data[prop_map["green"]]
		var blue_val: float = data[prop_map["blue"]]
		
		# If uchar, normalize [0, 255] to [0, 1]
		var red_type: String = properties[prop_map["red"]]["type"]
