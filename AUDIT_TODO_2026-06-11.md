...
- [ ] **D1. Typage strict** : ~1522 `var x = ...` non typés au départ. ✅ *Fichiers du cœur traités le 11/06 (~160 annotations, 0 restant)* : `foveacore_manager.gd`, `gpu_culler_pipeline.gd` (+ `ceili()` pour workgroups), `fovea_core_splat_renderer.gd` (+ `-> void` sur 3 signatures), `fovea_instanced_splat_renderer.gd` (+ membre `triangle_mesh_generator: GDScript`). Règles suivies : retours dynamiques (`get_shader_parameter`, `camera.attributes`) typés `Variant`, instances GDExtension (`ClassDB.instantiate`) typées `Object`, classes internes vérifiées membre par membre. ✅ *2e lot (11/06)* : subsystems (`fovea_splat_subsystem`, `fovea_foveated_subsystem`, `fovea_vr_subsystem`), `fovea_instanced_culler.gd` (39), `fovea_splat_cleaner.gd` (46), `fovea_splattable.gd` (33) — soit ~282 annotations au total, 0 restant dans le cœur du pipeline de rendu. ✅ *Corrections Godot 4.3 (20/06)* : suppression de la syntaxe non supportée `Dictionary[Key, Value]` (rétrogradée en `Dictionary` avec commentaires types) dans `fovea_clay_deformer.gd`, `fovea_segmentation_bridge.gd`, et `style_engine.gd`. Résolution des problèmes de chargement des Autoloads en preloadeant `fovea_dependency_manager.gd` dans `reconstruction_manager.gd` et `studio_to_3d_panel.gd`. **Reste** : ~1230 occurrences.
- [x] **D2. 11 appels `rd.sync()`** dans les pipelines GPU → stalls CPU-GPU chaque frame. ✅ *Corrigé le 20/06* : Regroupement des soumissions de compute shader (Culling + Precompute + Sort) dans `gpu_culler_pipeline.gd` pour éviter les syncs intermédiaires. Réduction de 3 syncs à 1 unique sync à la fin du frame (en mode classique) et 0 sync en mode asynchrone (`skip_sync`).
