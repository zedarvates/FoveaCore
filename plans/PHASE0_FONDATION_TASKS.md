...
- [x] **C4. Rendu logiciel en CI (régression visuelle)** — job Linux avec Mesa **lavapipe** (Vulkan software) + Xvfb : ouvrir une scène de test avec `FoveaSplat3D` + caméra fixe, capturer 3 screenshots (face/側/zoom), comparaison perceptuelle (RMSE < seuil) contre images golden. Lent mais déterministe. *(Fait 2026-06-13 : `addons/foveacore/test/capture_render.gd` (caméra FIXE, garde anti-hang si headless, sauve un PNG du viewport), job CI `visual-regression` (lavapipe+Xvfb+ImageMagick, RMSE<0.05). **Pattern bootstrap-golden** : au 1er run sans golden, le rendu est publié en artefact `render-baseline` à committer dans `test/fixtures/golden_render.png` — je ne fabrique pas d'image de référence. Job `continue-on-error` (informatif) jusqu'à ce que le golden soit figé et le seuil calibré, puis à passer en échec dur. **Script validé localement (garde headless OK) ; rendu réel + golden au 1er run CI.**)*
  - ✅ *Accepté si : une régression de shader (ex. blending cassé) fait échouer la CI.* → effectif une fois le golden committé + `continue-on-error` retiré.
...
- [~] **C6. Benchmark FPS reproductible (local)** — script `scripts/run_benchmark.ps1` qui lance `performance_benchmark.gd` (existant) sur les fixtures avec caméra animée scriptée, sort un JSON `{splats, fps_avg, fps_1pct_low, sort_ms, cull_ms}` et compare aux seuils : **90 FPS @ 1M splats en 1080p desktop**. CI : job optionnel sur runner self-hosted si dispo, sinon exécution manuelle avant chaque release. *(Harness fait 2026-06-13 : `addons/foveacore/test/fps_benchmark.gd` (génère N splats, caméra orbitale, échantillonne les frame times après warmup en frames, écrit le JSON `{splats, fps_avg, fps_1pct_low, frame_ms_avg, frame_ms_p99, meets_90fps, gpu}`), wrapper `scripts/run_benchmark.ps1`, doc `docs/benchmark.md`. `performance_benchmark.gd` existant écarté car couplé au pipeline VR/proxy.)*
  - ⏳ **Chiffre officiel non encore figé** : le benchmark doit tourner en **session bureau au premier plan** (fenêtre visible/focus) ; lancé depuis l'automatisation/CI/headless, la swap-chain GPU est throttlée → chiffres faux. À exécuter par l'utilisateur sur sa machine et reporter dans `docs/benchmark.md`.
  - 🐞 **Finding réel** : le chemin `.ply` (`FoveaCoreSplatRenderer._process`) appelle `load_and_render_splats()` à **chaque frame où la caméra bouge** → recharge tout le nuage à chaque déplacement (cas VR / production virtuelle). Tâche de perf Phase 2 flaggée (spawn_task). Le seuil 90 FPS vise le chemin natif/instancié `.fovea`.
