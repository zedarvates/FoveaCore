# 🎥 Prompts IA pour Vidéos de Reconstruction 3D (3DGS / NeRF)

Ces prompts sont optimisés pour les générateurs de vidéo IA (Kling, Luma Dream Machine, Runway Gen-3, Wan 2.1, Sora) afin d'obtenir des vidéos de type **turntable 360°** ou **survols de drone** photoréalistes, stables et géométriquement rigides, idéales pour l'import et la reconstruction dans votre plugin.

---

## 💡 Règles d'Or pour la Génération de Vidéos de Reconstruction

Pour éviter le phénomène de **"tâches informes" (blobs)** dû aux déformations et au morphing de l'IA :
1. **Définir la rigidité** : Insister sur le fait que l'objet est totalement rigide, statique et ne change pas de forme.
2. **Caméra stable** : Demander un mouvement d'orbite à vitesse, hauteur et distance constantes.
3. **Fond neutre** : Utiliser un fond blanc de studio uni, sans ligne d'horizon ou de sol visible, pour faciliter le détourage.
4. **Éclairage uniforme** : Un éclairage studio doux et neutre (pas de reflets de lumière qui bougent sur l'objet).

---

## 🌊 1. NOUVEAU - Catégorie : Dynamique & Fluides (Rivière & Cascade)
...
### Option C : Orbite Golden Hour (Esthétique / Test de Réflectivité)
*Idéal pour valider le rendu atmosphérique et la lumière rasante sur les volumes.*
> **Prompt :** An aerial drone scan orbiting a large medieval castle at golden hour. The sunlight is consistent and low, casting long, static shadows across the stone walls. The drone flies in a perfect circular path around the castle at a constant distance and speed. Static scenery, no wind, no moving elements. High dynamic range, photorealistic, sharp focus on architectural details, no warping, 4k.
