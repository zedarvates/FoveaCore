...
| PLY / Gaussian-splat workflow | Available with validation | The public-node/delegate contract passed 26/26 with a real one-splat PLY load, and representative PLY fixtures produced D3D12 captures. Each new asset and target GPU still requires validation. |
