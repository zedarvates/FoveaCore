...
- [x] **Spatial Chunking & Streaming**: CPU-side block culling (4096 Morton splats) against camera frustum, distance/gaze priority, deterministic per-frame splat admission, and file RAM caching with LRU eviction.
...
- [x] **Delta-Splat Variants (Morphs & Overrides)**: Sparse position/color/normal variants, stable asset/index IDs, client-owned revision state, defensive checkpoints, atomic rollback/replay, renderer packing, VRAM admission, and RID residency are implemented. Cross-client conflict resolution remains a separate integration gate.
...
- [ ] **Out-of-Core VRAM Streaming**: Direct SSD-to-VRAM transfer remains unimplemented. The current client path loads file chunks asynchronously into bounded CPU RAM and now limits scheduled splats per update before renderer upload.
